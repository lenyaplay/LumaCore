...
Этот документ — инженерная спецификация: конкретные решения там, где продуктовое ТЗ
сформулировано декларативно («общий интерфейс рендер-пайплайна», «кросс-компиляция
FFmpeg», ECDSA), плюс явные пометки рисков и мест, требующих прототипирования, а не
выдаваемых за решённые.

Целевые платформы: iOS + Android + Windows.

## Обновление по факту реализации (Android)

Android-пайплайн (`docs/ai_plans/04-android-camerax-gl-pipeline.md`,
`05-android-recording-performance-fix.md`) реализован и проверен на реальном устройстве
(не эмуляторе). Часть рисков, зафиксированных ниже как «не проверено», теперь имеет
конкретный ответ:

- **§1 (threading)**: push-модель `SurfaceTexture` → `GLRenderThread` → отдельный
  `EncodeThread` подтверждена рабочей как описано. Найдена деталь, которой не было в
  исходном плане: `SurfaceTexture.getTransformMatrix()` обязателен для правильной
  ориентации кадра камеры в шейдере — без него превью/запись выходят
  повёрнутыми/отражёнными.
- **§2 (GPU render pipeline)**: `GLRenderBackend` реализован полностью (все 5 эффектов —
  цветокоррекция, сепия, комикс/edge-detection, виньетка, частицы). Дополнительно
  найдено: `glReadPixels` отдаёт строки framebuffer'а снизу-вверх, а NV12/H.264 ожидает
  сверху-вниз — превью (показывается через `eglSwapBuffers`, без CPU-readback) этой
  проблемы не касается, но записанное видео без явного переворота при упаковке
  получается перевёрнутым по вертикали.
- **§4 (FFmpeg)**: `h264_mediacodec` принимает CPU NV12-буфер (не GPU-resident Surface) —
  подтверждено, как и предполагался базовый путь. Дополнительно найдено: при
  `AV_CODEC_FLAG_GLOBAL_HEADER` (всегда true для MP4-мукса) кодеку внутри себя нужны
  bitstream filters `h264_metadata`/`extract_extradata`, которые общий
  `--disable-bsfs` выключает — без точечного `--enable-bsf=...` `avcodec_open2` падает с
  «Bitstream filter not found».
- **§8, риск №4 (флаги минимальной сборки FFmpeg)** — для Android снят как разрешённый,
  см. предыдущий пункт про BSF.
- **Найдено, но не было предметом явного риска в этом документе**: синхронный
  FFmpeg/MediaCodec-энкод на GL/render-потоке (там, где `submitFrame()` изначально
  вызывался) душит и превью, и запись одновременно — исправлено переносом энкода на
  отдельный поток (Android-only, `EncoderSession.cpp`); и гонка потоков между
  audio-capture-потоком и encode-потоком за `audioAccum`/`audioAccumSamples` в том же
  файле — тот же риск теоретически существует и в iOS-ветке (общий файл), просто не
  проявился там при более редкой конкуренции.

Подробности каждого пункта, включая конкретные сообщения об ошибках и как их
диагностировали — `docs/ai_plans/04-android-camerax-gl-pipeline.md` и
`docs/ai_plans/05-android-recording-performance-fix.md`; практический пересказ для
разработчиков — `docs/DEVELOPMENT.md`.

## Ключевые отклонения от продуктового ТЗ (сознательные, зафиксированы явно)
...
- **Аудио**: запись звука входит в MVP, не только видео — иначе демо неполноценно для
  мультимедийного сценария, а A/V-синхронизация/буферизация — значимая часть требований
  к системе. См. §1.
...
отложено — не в скоупе этого проекта). Общее в C++:
...
закрытым приложением без покупки коммерческой лицензии x264.com.
...
используются — вендор-лок для проекта, который должен запускаться на
...
- iOS: `identifierForVendor` не подходит (сбрасывается при удалении приложения) —
  ...
  осознанно отложена (не в скоупе этого проекта).
...
LumaCore/
  README.md
  docs/                            # ARCHITECTURE.md, DEVELOPMENT.md, ai_plans/
  native/                         # см. §5
...
1. **iOS FFmpeg + Metal + CVPixelBuffer zero-copy цепочка — самая рискованная область.**
   ...
   прототипировать до того, как оценка «1–1.5 недели» в роадмапе будет считаться
   реалистичной.
2. **Интеграция текстур Flutter на iOS структурно отличается от Android** (push vs
   pull, см. §3), а не симметрична, как может показаться на первый взгляд. Плавный
...
7. **Vulkan-boilerplate аддитивен поверх обоих interop-рисков, а не отдельная статья
   ...
   фазой, а не в рамках того же бюджета на платформу, что GL/Metal.
...
есть нативно). Это отдельная будущая задача, не текущий скоуп; GL ES на Android и так
закрывает целевой список платформ («OpenGL ES / Vulkan / Metal» по одному нативному API
на платформу); сейчас ретрофит утроил бы Vulkan-поверхность без новой пользы. Одна фраза
в README «что дальше» — не более, и точно не реализация.

---

## Соответствие ключевым техническим требованиям

Все решения выше остаются в рамках таблицы «требование → реализация» из корневого
[README.md](../README.md) — этот документ детализирует *как*, не меняет *что*. Единственные
содержательные отклонения от продуктового ТЗ зафиксированы в начале документа.
