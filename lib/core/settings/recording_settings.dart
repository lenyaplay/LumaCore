...
class RecordingSettings {
  ...
  RecordingSettings copyWith({int? bitrateKbps, RecordingResolutionPreset? resolutionPreset}) => RecordingSettings(
...
class RecordingSettingsNotifier extends StateNotifier<RecordingSettings> {
  RecordingSettingsNotifier()
      : super(const RecordingSettings(bitrateKbps: 6000, resolutionPreset: RecordingResolutionPreset.hd1080));

  void setBitrate(int kbps) => state = state.copyWith(bitrateKbps: kbps);
  void setResolution(RecordingResolutionPreset preset) => state = state.copyWith(resolutionPreset: preset);
}

final recordingSettingsProvider =
    StateNotifierProvider<RecordingSettingsNotifier, RecordingSettings>((ref) => RecordingSettingsNotifier());
